...
extension CloudSync {
    func verifyAndSync(verificationCompletion: ((Error?) -> Void)?) {
        assert(Thread.isMainThread)

        os_log("Verifying account and syncing (modify + fetch)", log: log, type: .info)

        verifyAccount(verificationCompletion: verificationCompletion) { [weak self] in
    ...
    func verifyAndFetch() {
        ...
        verifyAccount(verificationCompletion: nil) { [weak self] in
            self?.fetchChanges()
        }
    }
}
...
private extension CloudSync {
    ...
    private func verifyAccount(verificationCompletion: ((Error?) -> Void)?,
                               andContinueWith block: @escaping () -> Void) {
        assert(Thread.isMainThread)
