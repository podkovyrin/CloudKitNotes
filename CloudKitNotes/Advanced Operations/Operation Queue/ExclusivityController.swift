...
/**
 ...
 `ANOperation` instances that have declared themselves as requiring mutual exclusivity.
 We use a singleton because mutual exclusivity must be enforced across the entire
 app, regardless of the `OperationQueue` on which an `ANOperation` was executed.
 */
class ExclusivityController {
    ...
    private var operations: [String: [ANOperation]] = [:]
    ...
    func addOperation(_ operation: ANOperation, categories: [String]) {
        /*
    ...
    func removeOperation(_ operation: ANOperation, categories: [String]) {
        serialQueue.async {
            for category in categories {
    ...
    // MARK: ANOperation Management

    private func noqueue_addOperation(_ operation: ANOperation, category: String) {
    ...
    private func noqueue_removeOperation(_ operation: ANOperation, category: String) {
